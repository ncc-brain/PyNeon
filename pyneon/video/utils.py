...
def marker_name_to_dict(marker_name: str) -> Tuple[str, object]:
    """
    Convert a marker name to a dictionary format with type identifier.
    ...
    ----------
    marker_name : str
        Name of the marker. Must be one of:
    ...
    ------
    ValueError
        If marker_name is not recognized or has invalid configuration.

    Examples
    --------
    >>> marker_type, dictionary = marker_name_to_dict('36h10')
    >>> marker_type
    'apriltag'

    >>> marker_type, dictionary = marker_name_to_dict('6x6_250')
    >>> marker_type
    'aruco'
    """
    # Check for AprilTag markers
    if marker_name in APRILTAG_SIZES:
        dict_name: str = f"DICT_APRILTAG_{marker_name.upper()}"
