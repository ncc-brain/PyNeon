...
class TestBIDS:
    ...
    def test_export_motion_bids_basic(self, request, dataset_fixture, extra_metadata):
        """Test basic Motion-BIDS export with default prefix."""
        dataset = request.getfixturevalue(dataset_fixture)
        with tempfile.TemporaryDirectory() as tmpdir:
            motion_dir = Path(tmpdir) / "motion"  # Doesn't need to be created
            not_motion_dir = Path(tmpdir) / "not_motion"
            ...
            for recording in dataset.recordings:
                ...
                default_prefix = f"sub-{recording.info['wearer_name']}_task-TaskName_tracksys-NeonIMU"
                ...
                assert (
                    ...
                    == recording.info["module_serial_number"]
                )
    ...
    def test_export_motion_bids_with_prefix(self, request, dataset_fixture):
    ...
    def test_export_eye_bids_basic(self, request, dataset_fixture, extra_metadata):
    ...
    def test_export_eye_bids_with_prefix(self, request, dataset_fixture):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            for recording in dataset.recordings:
                ...
                # Test invalid prefix
                with pytest.raises(
                    ValueError, match="Prefix must contain 'sub-<label>'"
                ):
